import mentorOneImage from '../../assets/mentors/inna-step-one-med.jpg';
import mentorTwoImage from '../../assets/mentors/nika-step-one-med.jpg';
import mentorThreeImage from '../../assets/mentors/ivanna-step-one-med.JPG';
import mentorFourImage from '../../assets/mentors/mentor-placeholder.png';
import mentorFiveImage from '../../assets/mentors/bohdan-step-one-med.jpg';
import mentorSixImage from '../../assets/mentors/viktor-step-one-med.jpg';
import mentorSevenImage from '../../assets/mentors/maxim-step-one-med.jpg';
import mentorEightImage from '../../assets/mentors/mentor-placeholder.png';
import mentorNineImage from '../../assets/mentors/mentor-placeholder-two.png';
import mentorTenImage from '../../assets/mentors/mentor-placeholder-three.png';

const team = [
  {
    id: 2,
    image: mentorOneImage,
    name: "Інна",
    position: "засновниця курсу, викладачка фармакології ЛДМУ"
  },
  {
    id: 3,
    image: mentorTwoImage,
    name: "Ніка",
    position: "лікар-інтерн невролог"
  },
  {
    id: 4,
    image: mentorThreeImage,
    name: "Іванна",
    position: "лікар-інтерн внутрішньої медицини"
  },
  {
    id: 5,
    image: mentorFourImage,
    name: "Катерина",
    position: "студентка 6го курсу"
  },
  {
    id: 6,
    image: mentorFiveImage,
    name: "Богдан",
    position: "лікар-інтерн внутрішньої медицини"
  },
  {
    id: 7,
    image: mentorSixImage,
    name: "Віктор",
    position: "лікар-інтерн хірург"
  },
  {
    id: 8,
    image: mentorSevenImage,
    name: "Максим",
    position: "лікар-інтерн анестезіолог"
  },
  {
    id: 9,
    image: mentorEightImage,
    name: "Олександр",
    position: "засновник курсу, студент 4го курс стомат факультету, випускник проєкту It’s Med Ed. Результат КРОК-1 — 93%"
  },
  {
    id: 10,
    image: mentorNineImage,
    name: "Катерина",
    position: "засновниця курсу, студентка 4го курс стомат факультету, випускниця проєкту It’s Med Ed. Результат КРОК-1 — 91,3%"
  },
  {
    id: 11,
    image: mentorTenImage,
    name: "Вікторія",
    position: "студентка 4го курс стомат факультету, випускниця проєкту It’s Med Ed. Результат КРОК-1 — 88%"
  },
];

export default team;
