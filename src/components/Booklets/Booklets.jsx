    ...
    const fetchBookletsData = async () => {
      try {
        const [testsRes, attemptsRes, savedIdsRes] = await Promise.all([
          api.get("/tests?type=BOOKLET"),
          api.get("/attempts/my-attempts"),
          api.get("/saved-questions/test-ids"),
        ]);

        const activeBooklets = (testsRes.data.data || testsRes.data).filter(
          (test) => test.status === "PUBLISHED",
        );

        setTests(activeBooklets);
        setAttempts(attemptsRes.data);
        setSavedTestIds(new Set(savedIdsRes.data));
      } catch (err) {
        console.error("Failed to fetch booklets data:", err);
        setError("Не вдалося завантажити буклети. Спробуйте пізніше.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="booklets">
      ...
      <DashboardLeft
        ...
        searchPlaceholder="Пошук серед букле..."
      />
      ...
      <main className="booklets__content">
        <div className="booklets__header-actions">
          <div className="booklets__search desktop-search-only">
            <input
              type="text"
              placeholder="Пошук серед буклетів..."
              value={searchQuery}
        ...
        {isLoading ? (
          <div className="booklets-loading">Завантаження буклетів...</div>
        ) : error ? (
